...
const links: NavLink[] = [
  { name: "Weby & software", href: "/#sluzby" },
...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 z-50 w-full transition-all ${scrolled ? "bg-[#0b0c0e]/80 backdrop-blur" : "bg-transparent"}`}>
      <MobileMenu mobileMenuOpened={mobileOpen} setMobileMenuOpened={setMobileOpen} links={links} />
      <div className="container mx-auto flex h-16 max-w-6xl items-center justify-between px-4 md:px-6">
        <Link href="/" className="text-lg font-extrabold tracking-tight text-primary" aria-label="Petr Vurm – úvod">Petr Vurm</Link>
        <nav className="hidden items-center gap-5 md:flex" aria-label="Hlavní navigace">
          {links.map((link) => <Link key={link.href} href={link.href} className={`text-sm transition-colors ${isActive(link.href) ? "text-primary" : "text-white/80 hover:text-white"}`} aria-current={isActive(link.href) ? "page" : undefined}>{link.name}</Link>)}
          <Link href="/kontakt" className="rounded-lg bg-primary px-3 py-1.5 text-sm font-semibold text-black transition hover:bg-primary/90">Probrat projekt</Link>
        </nav>
